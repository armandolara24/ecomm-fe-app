...
export class ProductListComponent implements OnInit {
  ...
  handleSearchProducts() {
    const tmpkeyword: string = this.route.snapshot.paramMap.get("keyword")!;

    /*
    // now search for products using keyword
    this.productService.searchProducts(tmpkeyword).subscribe(
      data => {
        this.products = data;
      }
    )
    */
    ...
    if (tmpkeyword != this.previousKeyword) {
      this.pageNumber = 1;
    }
    this.previousKeyword = tmpkeyword;
    //console.log(`keyword=${keyword}, pageNumber=${this.pageNumber}`)

    this.productService.searchProductsPaginate(this.pageNumber - 1,
      this.pageSize, tmpkeyword).subscribe(this.processResult())
  }
  ...
  addToCart(product: Product) {
    ...
    let cartItem = new CartItem(product.id, product.name, product.imageUrl, product.unitPrice);
